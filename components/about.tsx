...
export default function AboutSheet() {
  ...
  if (!isOpen) {
    return (
      <Button onClick={() => setIsOpen(true)} className="w-full mb-4 p-6 bg-slate-800 text-white hover:bg-slate-700">
  ...
  return (
    <div
      className={`fixed inset-0 bg-black/70 z-[9999] flex items-center justify-center overflow-y-auto transition-all duration-300 ${isAnimating ? "opacity-0" : "opacity-100"}`}
      onClick={handleClose}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        className={`bg-white max-w-4xl max-h-[90vh] overflow-y-auto w-full relative transition-all duration-300 ${isAnimating ? "scale-95 translate-y-4" : "scale-100 translate-y-0"}`}
        ...
        <div className="p-6">
          ...
          <section className="mt-10">
            ...
            <ul className="list-disc list-inside ml-2 space-y-1 text-gray-700 text-sm">
              ...
              <li>
                Clements, J. F., Rasmussen, P. C., Schulenberg, T. S., et al. (2024). The eBird/Clements checklist of Birds of the World: v2024. Cornell Lab of Ornithology. 
                <a href="https://www.birds.cornell.edu/clementschecklist/download/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline ml-1">
                  Download link
                </a>
              </li>
            </ul>
